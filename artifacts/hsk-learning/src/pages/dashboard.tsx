      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-semibold text-sm text-foreground">Tính năng mới</h2>
          </div>
          <div className="space-y-2.5">
            {NEW_FEATURES.map(({ icon: Icon, label, desc, href, badge, color, badgeColor }) => (
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-sm text-foreground">Thư viện</h2>
            </div>
            <Link href="/learn/1">
              <span className="text-xs text-primary font-medium hover:underline">Xem tất cả →</span>
            </Link>
          </div>

          {/* Category grid */}
          <div className="grid grid-cols-4 gap-2 mb-3">
            {LIBRARY_SECTIONS.map(({ label, icon: Icon, desc, color, href }) => (
          ...
          {recentLessons.length > 0 && (
            <div className="bg-card border rounded-2xl overflow-hidden">
              <div className="px-4 py-3 border-b">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Bài học gần đây</p>
              </div>
                ...
                <Link key={lesson.id} href={`/lesson/${lesson.id}`}>
                  <div className={cn(
                    ...
                    <div className={cn(
                      ...
                      lesson.isCompleted ? "bg-green-100 text-green-600" : "bg-primary/10 text-primary",
                    )}>
                      {lesson.isCompleted ? <CheckCircle className="w-4 h-4" /> : <BookOpen className="w-4 h-4" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">{lesson.title}</p>
                      <p className="text-[11px] text-muted-foreground">{lesson.titleChinese} · HSK {lesson.level}</p>
                    </div>
